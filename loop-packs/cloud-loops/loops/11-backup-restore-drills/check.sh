...
done=$(aws backup list-restore-jobs --by-created-after "$since" --query "length(RestoreJobs[?Status=='COMPLETED'])" --output text 2>/dev/null) || { echo "cannot read AWS Backup"; exit 1; }
