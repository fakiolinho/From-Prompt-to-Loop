...
n=$(aws ce get-anomalies --date-interval StartDate=$since,EndDate=$(date -u +%Y-%m-%d) --query 'length(Anomalies)' --output text 2>/dev/null) || { echo "cannot read Cost Anomaly Detection (enable it; ce is us-east-1)"; exit 2; }
