...
flips=$(aws cloudwatch describe-alarm-history --history-item-type StateUpdate --start-date "$since" --query 'length(AlarmHistoryItems)' --output text 2>/dev/null) || { echo "cannot read CloudWatch"; exit 1; }
