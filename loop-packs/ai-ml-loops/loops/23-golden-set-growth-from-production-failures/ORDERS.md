...
`loops/23-golden-set-growth-from-production-failures/check.sh` exists in your project, you have.
...
the standing orders to your `CLAUDE.md` without replacing what is already there.
    ...
    ./install.sh ~/code/my-app 23

**Yes.** From your project root:

    ./run-loop.sh 23 --check    is there work? Never wakes an agent.
    ./run-loop.sh 23            if there is work, hand it to the agent.
